...
Configure `CAIL_IDENTITY_JWKS` (JSON), `CAIL_IDENTITY_ISSUER=https://tools.ailab.gc.cuny.edu/cail-sso`, `PUBLIC_URL=https://tools.ailab.gc.cuny.edu/slide-maker`, and a reviewed `CAIL_GATEWAY_URL`. The canonical identity issuer includes `/cail-sso`; the Gateway base URL is `https://tools.ailab.gc.cuny.edu` without that path. These values serve different protocols and must not be interchanged. The web production build uses `/slide-maker`; the Node API still receives `/api/...` behind the existing path-stripping boundary. The current Node process must remain loopback-bound. This change deliberately does not introduce a private-network proxy or claim a transport decision for Node has been approved.
