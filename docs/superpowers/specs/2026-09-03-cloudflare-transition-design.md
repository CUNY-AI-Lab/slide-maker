...
tools.ailab.gc.cuny.edu/slide-maker/…  ──▶  cail-doorway  ──service binding──▶  cail-slide-maker (Hono)
                                              (CUNY Login, admission,          ├── assets   apps/web/build, base path /slide-maker
                                               ...
                                               x-cail-gateway-identity-jwt)    │            document · mutation log · lock · presence · WebSocket fan-out
                                                                               ...
                                                                               └── CHAT_LIMIT · EXPORT_LIMIT · UPLOAD_LIMIT   (Rate Limiting bindings, keyed by subject)
...
| Admin role | A small var of admin subjects (`SLIDE_MAKER_ADMIN_SUBJECTS`) | Read `accessRole` from the doorway | The doorway deliberately does not forward roles; the admission admin desk keeps its own audience for the same reason. The app's remaining admin surface (deck stats, publish) is small |
...
| Rate limits | Rate Limiting bindings keyed by subject | `RateLimiterMemory` | Global and durable across deploys; behind a service binding the client IP is not the right key anyway |
...
// apps/api/wrangler.jsonc
{
  ...
  "ai": { "binding": "AI" },
  "services": [{ "binding": "GATEWAY", "service": "cail-model-api" }],
  "d1_databases": [{
...
Secrets are set with `wrangler secret put`, never in `vars`: `CAIL_IDENTITY_JWKS` (the doorway's published key set, as the other tools carry it), `SLIDE_MAKER_ADMIN_SUBJECTS`, `PEXELS_API_KEY`, `TAVILY_API_KEY`, `BRAVE_API_KEY`. Rate Limiting bindings count over 10 or 60 second periods, so the `period` values above are the binding half of any longer window; nothing in the app needs a 15-minute window once passwords are gone.
...
export function identity(env: Env) {
  ...
  return async (c, next) => {
    ...
    c.set('isAdmin', env.SLIDE_MAKER_ADMIN_SUBJECTS.split(',').includes(who.subject))
    await next()
  }
}
```
...
export class DeckRoom extends DurableObject<Env> {
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    ctx.storage.sql.exec(`
      ...
      CREATE TABLE IF NOT EXISTS mutations (version INTEGER PRIMARY KEY, actor_kind TEXT, actor_subject TEXT, message_id TEXT, mutation TEXT, inverse TEXT, created_at INTEGER);
  ...
  async apply(batch: { baseVersion: number; mutations: Mutation[]; actor: Actor; mode: 'commit' | 'propose' }) {
    // runs @slide-maker/core applyBatch against the stored document, appends log rows,
    // bumps version, broadcasts { type: 'mutation', ... } to every hibernating WebSocket
  }
...
- The dropdown is `GET /v1/models` with the same bearer, filtered by `SLIDE_MAKER_MODEL_POLICY`. `tools-required` keeps only entries whose `capabilities` include `function-calling`, which is what the mutation tool needs; the fenced-JSON fallback from PR #11 stays for a policy of `all`. Workers AI models documented with function calling today include `@cf/zai-org/glm-4.7-flash`, `@cf/openai/gpt-oss-120b`, `@cf/qwen/qwen3-30b-a3b-fp8`, `@cf/google/gemma-4-26b-a4b-it`, and `@cf/mistralai/mistral-small-3.1-24b-instruct`.
...
| Admin approval queue, `users.status`, token caps | Admission owns access; `SLIDE_MAKER_ADMIN_SUBJECTS` for the app's admin page; gateway quota for budgets |
...
| `rate-limiter-flexible` in memory | Rate Limiting bindings keyed by subject |
...
| SSRF guard using `node:dns` and `node:net` | Same code; both modules are available under Node compatibility |
...
plus a `SLIDE_MAKER` service binding in the doorway's Wrangler configuration, and optionally a binding in admission's fleet probe so the app appears on the systems page. The doorway forwards the full path, as it does for Agent Studio, which is why the app keeps its `/slide-maker` base path.
...
Each phase is deployable on the current host until Phase 5 flips traffic. Sizes are relative.
...
### Phase 3 — Identity (M)
- Identity middleware, `users` keyed by subject, display-name prompt, share-by-name and capability links, admin subjects var, legacy claim endpoint, health route, `cail-log` events.
- Delete `routes/auth.ts`, Lucia, argon2, SES, the approval queue, and the token-cap editor.
- Open the doorway pull request (§2.7). Until it merges, staging runs with the test issuer.
...
| Claude is not in the lab catalog | The mutation tool is exercised against `glm-4.7-flash`, `gpt-oss-120b`, and `gemma-4` on staging with the golden fixtures before the default is chosen; the fenced fallback covers models without tools; a direct Anthropic path stays possible in the package if the lab decides to admit one |
...
The 7 layouts and 14 modules, the zone model, the export zip layout, the sharing roles (owner, editor, viewer), the CSP and sanitization invariants in `CLAUDE.md`, the SSRF guard, the path traversal guard, and every package proposed in PR #11. This document changes the host, the identity boundary, and the model-access wiring, nothing above them.
