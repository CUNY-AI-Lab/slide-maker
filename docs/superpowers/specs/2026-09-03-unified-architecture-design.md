...
**Revision 2026-09-07:** responds to the review on PR #11. Each phase now has exactly one write path (the dual-write transition is gone); the step that moves deck data is named as its own boundary with a verified round trip and a rollback (§3, Phase 2); the fenced-JSON parser and the `chars / 4` estimate are deleted at Phase 5 rather than kept as fallbacks; commit and version semantics are shared word for word with the companion design (§2.2, PR #12 §2.4).
**Review scope:** this is a design document. The CI check on the pull request shows that the unchanged application still builds and tests; it validates nothing about the architecture proposed here. The first validation of the design is the Phase 0 golden test, and the first irreversible step is Phase 2, each of which needs its own approval before it runs.
...
| AI output | Provider tool use with a JSON schema generated from the `Mutation` zod type. Models without function calling are not offered in the model list | Fenced blocks parsed by regex on both ends; or tool use with the fenced parser kept as a fallback | Schema-validated input, real usage numbers, server-side application. A retained fallback is a second parser to keep in parity with the first, which is the class of duplication this document exists to remove |
...
deck_commits    (deck_id, version, base_version, actor_kind ENUM(user|ai|plan|system), actor_id, message_id NULL,
                 mutations JSON, inverses JSON, created_at, PRIMARY KEY (deck_id, version))
```

- **Commit and version semantics** (shared with PR #12 §2.4, which stores the same row inside a per-deck Durable Object with `version INTEGER PRIMARY KEY` because the object is the deck): a commit is one batch, whatever its length. `decks.version` increases by exactly one per commit. The log row for a commit is keyed by `(deck_id, version)` where `version` is the value after the commit, and it carries the whole batch: `mutations[]` in application order and `inverses[]` in the order that undoes them. There is no per-mutation version and no per-mutation row. A request's `baseVersion` must equal `decks.version` at commit time; anything else is `409 version`.
- `mode: 'propose'` runs the reducer and returns `{ mutations, inverses }` without touching `decks` or the log. Accepting a proposal is an ordinary commit with the then-current `baseVersion`.
- `slides` and `content_blocks` are read once by the Phase 2 migration and then dropped.
...
- Undo/redo is a stack of commits; undo dispatches a commit's `inverses` as a new batch, so it produces a new commit and a new version like any edit. Per-AI-message undo is the inverses of the commits sharing a `message_id`, newest first.
...
- Token usage comes from provider `usage` events only. A stream that ends without a usage event is logged with `usage: null` and surfaced in the transcript diagnostics as an adapter defect; nothing estimates.
...
- Replace the two fence regexes with one parser in `packages/shared`, used by both the client and `chat.ts` (an immediate bug fix). This parser is the only AI edit path until Phase 5 replaces it with tool use and deletes it; it does not survive as a fallback.
...
### Phase 1 — Core reducer; the tables remain the only store (M)
- Create `packages/core`: zod schemas, reducer, inverses, `computeThemeVars`, `resolveArtifactBlock`, property tests.
- Add `POST /api/decks/:id/mutations`. In this phase it loads the deck from `slides` + `content_blocks` into a `DeckDocument` with `loadDeck(id)`, runs the reducer, and writes the result back to the same two tables in one raw-SQLite transaction (the touched slides' rows are replaced). There is no `document` column yet and nothing is written twice: the tables are the store, the reducer is the only thing that writes them. `decks.version` is added and bumped per commit, and `deck_commits` is created and written from here on, so the log starts one phase before the document does.
- The 14 legacy endpoints remain in place but every client caller moves off them in this phase: `dispatch()` replaces `applyMutation` and `applyMutationSilent`; `history.ts` becomes a commit stack; the components that call `apiCall` or `fetch` directly (`SlideRenderer`, `SlideCard`, `BlockItem`, `AddSlideMenu`, `ModulePicker`, `CanvasToolbar`, `TemplatesTab`) switch to `dispatch`. The legacy endpoints therefore carry no traffic when Phase 2 deletes them; they are not maintained as a second write path.

### Phase 2 — Document cutover: the data-preservation boundary (M)
...
- **Switch.** `GET /api/decks/:id` returns the document; chat, preview, export, and plan read it; the mutations endpoint writes `decks.document` only. Delete the 14 slide/block endpoints, `plan/apply`, the five join copies, `normalizeBlockData`, and `resolve-artifacts.ts`.
- **Drop.** `slides` and `content_blocks` are dropped one release cycle later. Until then the rollback is: restore the backup, redeploy the Phase 1 build. After the drop, the backup is the archive.
What is preserved and how: nothing is deleted before the round trip verifies every deck; the backup outlives the release cycle; every deck's first commit row records the version at which its document was born; the property test on the reducer guarantees that `document` plus the log can reproduce any later state.
...
- Delete `pending-mutations.ts` (`mode: 'propose'` replaces it), the Phase 0 fenced parser, and the `chars / 4` accounting. In the same change the model list drops every entry without function calling.
...
| Whole-document writes and log growth | Decks are capped at 60 slides, so documents are tens of kilobytes; compact `deck_commits` past N rows per deck on export or by a scheduled job |
| The Phase 2 cutover loses or corrupts deck data | Round trip verified for every deck before any write; file backup taken under freeze; tables dropped one release cycle later; rollback is restore plus redeploy (§3, Phase 2) |
...
| Tool-use fidelity on OpenRouter models | Before Phase 5 ships, every listed model is run through the tool path against the golden fixture prompts on staging, using the existing `/api/debug/transcripts` log; a model that fails is removed from the list rather than given a second parser |
