...
import { logRequestFailure } from './fleet/log.js'
import { readinessRouter } from './fleet/readiness.js'
import { loadIdentityConfigs } from './auth/identity.js'
...
app.onError((err, c) => {
  ...
  logRequestFailure(correlationFromHeaders(c.req.raw.headers).request_id)
...
app.route('/internal', readinessRouter(() => {
  sqlite.prepare('SELECT canonical_subject FROM users LIMIT 1').all()
}, loadIdentityConfigs))
