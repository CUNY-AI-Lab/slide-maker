...
- `updateBlock`, `removeBlock`, `removeSlide`, `updateSlide`, `setTheme`, `updateTheme` (partial theme patch — colors/fonts merged), `updateArtifactConfig`

System prompt at `apps/api/src/prompts/system.ts` — `buildSystemPrompt()` returns `{ staticPrompt, dynamicContext }`. The static half (role, mutation format, module/layout reference, examples) is stable across requests; the dynamic half holds per-request deck state, templates, files, artifacts, render diagnostics. Anthropic provider (`apps/api/src/providers/anthropic.ts`) splits them into two system text blocks and sets `cache_control: { type: 'ephemeral' }` on the static one to hit Anthropic prompt caching. Do not move dynamic content into `staticPrompt` — it poisons the cache. OpenRouter and Bedrock providers concatenate both halves (no caching support).

The prompt includes deck state, templates, theme, and **uploaded file URLs** so AI can reference them in image modules. The AI does NOT have web access — it directs users to `/search` command for web images.
...
- **Deck-lock enforcement.** The `checkDeckLock()` helper (`apps/api/src/middleware/deck-lock.ts`) gates every mutating slide/block endpoint (CRUD, reorder, cross-zone move, plan apply) — non-holders get `409 { error, lockedBy: { name, since } }`. Expired and self-owned locks pass through. Metadata routes (PATCH/DELETE `/api/decks/:id`) are NOT gated so the owner can always rename/delete. Client-side 409 handling lives in `apiCall()` at `apps/web/src/lib/utils/mutations.ts:33-39`: on 409 it parses `lockedBy.name`, writes a 5-second toast via the `lockConflictMessage` store, and returns `null` from the mutation. The optimistic store update still persists locally until reload — rolling back the in-memory mutation after a 409 is a remaining improvement.
...
- No real-time collaborative editing — uses pessimistic locking (5-min TTL with heartbeat) and polling-based presence (30s). Mutation endpoints are server-gated via `checkDeckLock()` (409 to non-holders); client shows a toast via `lockConflictMessage` but does not yet roll back the optimistic local mutation.
